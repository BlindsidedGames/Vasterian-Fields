...
namespace VoxelBusters.EssentialKit.GameServicesCore.Android
{
    public class NativeGameServices : NativeAndroidJavaObjectWrapper
    {
        ...
        public void Authenticate(bool interactive, NativePlayerAuthenticationListener listener)
        {
            Activity.RunOnUiThread(() => {
#if NATIVE_PLUGINS_DEBUG_ENABLED
                DebugLogger.Log("[Class : NativeGameServices][Method(RunOnUiThread) : Authenticate]");
#endif
                Call(Native.Method.kAuthenticate, new object[] { interactive, listener } );
            });
        }
        ...
        public void LoadServerCredentials(string serverClientId, bool forceRefreshToken, int[] additionalScopes, NativeLoadServerCredentials listener)
        {
            Activity.RunOnUiThread(() => {
#if NATIVE_PLUGINS_DEBUG_ENABLED
                DebugLogger.Log("[Class : NativeGameServices][Method(RunOnUiThread) : LoadServerCredentials]");
#endif
                Call(Native.Method.kLoadServerCredentials, new object[] { serverClientId, forceRefreshToken, additionalScopes, listener } );
            });
        }
